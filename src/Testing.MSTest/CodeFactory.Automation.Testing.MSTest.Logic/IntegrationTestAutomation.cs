...
namespace CodeFactory.Automation.Testing.MSTest.Logic
{
    /// <summary>
    /// Automation logic that supports integration testing using the MSTest unit test framework.
    /// </summary>
    public static class IntegrationTestAutomation
    {
        /// <summary>
        ...
        private static List<CsMethod> GetAllInterfaceMethods(this CsInterface source)
        {
            var result = new List<CsMethod>();

            if (source == null) return result;

            if (source.Methods.Any()) result.AddRange(source.Methods);
            ...
            if (source.InheritedInterfaces.Any())
            {
                foreach (var inheritedInterface in source.InheritedInterfaces)
                {
                    ...
                    if (inheritedMethods.Any()) result.AddRange(inheritedMethods);
                }
            }
