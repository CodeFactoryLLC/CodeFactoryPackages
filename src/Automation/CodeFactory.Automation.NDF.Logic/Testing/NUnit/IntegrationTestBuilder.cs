...
namespace CodeFactory.Automation.NDF.Logic.Testing.NUnit
{
	...
	public static class IntegrationTestBuilder
	{
		...
		public static async Task UpdateTestAsync(this IVsActions source, CsInterface contract, CsSource testClassSource)
		{
			...
			foreach (var addMethod in AddTests)
			{
				...
				else
				{
					...
					testMethodFormatter.AppendCodeLine(4, $"await Assert.ThrowsAsync<NotImplementedException>({asyncStatement} ()=> {awaitStatement} _contract.{addMethod.Name}({methodParameters}));");
				}
		...
		/// <returns>True if configured with NUnit Tests, false if not.</returns>
		/// <exception cref="CodeFactoryException">Thrown if required data is missing.</exception>
		public static async Task<bool> TestProjectIsConfiguredNUnitAsync(this VsProject project, bool throwError = false)
		{
